...
class m221021_071303_create_booking_table extends Migration
{
    ...
    public function safeUp()
    {
        $this->createTable('{{%booking}}', [
            'id' => $this->primaryKey(),
            'localtion_province' => $this->string()->comment('จังหวัด'),
            'localtion_district' => $this->string()->comment('อำเภอ'),
            'passengers_number' => $this->integer()->comment('จำนวนผู้โดยสาร'),
            'car_van' => $this->integer()->comment('รถตู้'),
            'car_truck' => $this->integer()->comment('รถกระบะ'),
            'car_sedan' => $this->integer()->comment('รถเก๋ง'),
            'car_bus' => $this->integer()->comment('รถบัส'),
            'car_small_truck' => $this->integer()->comment('รถบรรทุกขนาดเล็ก'),
